...
/*   Updated: 2021/04/12 22:52:00 by swquinc          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(Form const &form)
{
	...
	{
		throw Form::UnsignedException(getName(), form.getName());
	}
	else if (getGrade() > form.getExecuteGrade())
	{
		std::cout << getName() << " cannot execute form \"" << form.getName() << "\" because beraucrate grade too low for execute this form" << std::endl;
		throw Form::GradeTooLowException(getName());
	}
	form.execute(*this);
}
