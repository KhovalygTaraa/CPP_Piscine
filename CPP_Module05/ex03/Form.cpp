...
/*   Updated: 2021/04/12 22:18:50 by swquinc          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Form::UnsignedException::UnsignedException(std::string name, std::string formName): name(name), formName(formName)
{
}
...
const char *Form::UnsignedException::what() const throw()
{
	...
	str = getName();
	str.append(" cannot execute \"");
	str.append(getFormName());
	str.append("\" because form unsigned!");
	return (str.c_str());
}

std::string const Form::UnsignedException::getName(void) const
{
	return (this->name);
}

std::string const Form::UnsignedException::getFormName(void) const
{
	return (this->formName);
}
